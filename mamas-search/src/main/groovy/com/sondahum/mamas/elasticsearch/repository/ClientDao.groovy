...
@Repository
class ClientDao extends EsClientHelper implements EsDao {

    private final String indexName = 'mamas-client'
    ...
    void update(EsDto esDto) {
        esDto.getItemList().each { item ->
            UpdateRequest request = new UpdateRequest(indexName, item.id) //
